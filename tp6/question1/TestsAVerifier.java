...
public class TestsAVerifier extends junit.framework.TestCase{
    ...
    public void testUneConstante(){
        try{
            assertEquals(m + "lecture m�moire ?, i==3 ?", 3, m.lire("i").intValue());
            assertEquals(m  + "lecture m�moire ?, j==5 ?", 5, m.lire("j").intValue());
    ...
    public void testUneVariable(){
        try{
            ...
            assertEquals(m  + "lecture m�moire ?, j==5 ?", 5, m.lire("j").intValue());
            m.ecrire("z",10);
            assertEquals(m  + "�criture m�moire ?, z==10 ?", 10, m.lire("z").intValue());
        }catch(Exception e){
            fail(" exception inattendue ???" + e.getClass().getName() + e.getMessage());
        }
    }
    ...
    public void testUneAddition(){
        try{
            ...
            fail(" StackOverflowError, appel r�cursif sans fin de " + m + " (i + 2) + i ???");
        }catch(Exception e){
            fail(" exception inattendue ???" + e.getClass().getName() + e.getMessage());
        }
    }
    ...
    public void testMultiplication(){
        try{
            ...
            fail(" StackOverflowError, appel r�cursif sans fin de " + m + " i * j ???");
        }catch(Exception e){
            fail(" exception inattendue ???" + e.getClass().getName() + e.getMessage());
        }
    }
    ...
    public void testDivision(){
        try{
            ...
            fail(" StackOverflowError, appel r�cursif sans fin de " + m + " i*j/i ???");  	
        }catch(Exception e){
            fail(" exception inattendue ???" + e.getClass().getName() + e.getMessage());
        }
        try{
